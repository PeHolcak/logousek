...
const useDrop = (dndContext: DndContextValueType | null) => {
  ...
  const onDropCardClick = useCallback(
    (index: number, avaibleCards?: CardsInGame) => {
      const item = avaibleCards?.find(
        (avaibleCard) => avaibleCard?.keyImage === dndContext?.selectedCard
      )
      if (item) {
