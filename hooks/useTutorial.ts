...
export const useTutorial = () => {
  ...
  const runTutorial = useCallback(() => {
    if (config.length) {
      if (getItemFromLocalStorage('wasRunned') !== 'true') {
        handleHiglighttedIds('remove', ['character'], [])
        setToLocalSotrage('wasRunned', true)
      }
      setIsTutorialOpened(true)
      next()
    }
  }, [config.length, handleHiglighttedIds, next])

  useEffect(() => {
    if (getItemFromLocalStorage('wasRunned') !== 'true' && _document) {
      handleHiglighttedIds('add', ['character'], [])
    }
  }, [_document, handleHiglighttedIds, runTutorial])
