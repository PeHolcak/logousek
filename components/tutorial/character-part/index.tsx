...
const CharacterPart: React.FC<CharacterPartProps> = ({
  ...
  const characterImgSrc = useMemo(() => {
    const foundCharacter = CHARACTERS_CONFIG.find(
      (character) => userSettingsContext?.character === character.name
    )
    return foundCharacter?.imageSrc ?? DEFAULT_CHARACTER
  }, [DEFAULT_CHARACTER, userSettingsContext?.character])

  const aura = useMemo(
    () => userSettingsContext?.aura ?? DEFAULT_AURA,
    [DEFAULT_AURA, userSettingsContext?.aura]
  )

  return (
    <Blob color={aura} isBlobShowed={true} name="TutorialBlob">
      <S.BlobContainer isTutorialOpened={isTutorialOpened}>
        ...
        >
          ...
          {isTutorialOpened ? <Reference reference={IMAGES_REFERENCE} /> : null}
        </S.CharacterWrapper>
      </S.BlobContainer>
    </Blob>
  )
