import { NamedImage, serialitySrcs } from '@helpers/image-helper'

export const threePictures = [
  [
    <NamedImage key="snowman-1" name="snowman-1" src={serialitySrcs.snowman} />,
    <NamedImage
      key="snowman-2"
      name="snowman-2"
      src={serialitySrcs.snowman2}
    />,
    <NamedImage
      key="snowman-3"
      name="snowman-3"
      src={serialitySrcs.snowman3}
    />,
  ],
  [
    <NamedImage key="banana-1" name="banana-1" src={serialitySrcs.banana} />,
    <NamedImage key="banana-2" name="banana-2" src={serialitySrcs.banana2} />,
    <NamedImage key="banana-3" name="banana-3" src={serialitySrcs.banana3} />,
  ],
  [
    <NamedImage
      key="banknote-1"
      name="banknote-1"
      src={serialitySrcs.banknote}
    />,
    <NamedImage
      key="banknote-2"
      name="banknote-2"
      src={serialitySrcs.banknote2}
    />,
    <NamedImage
      key="banknote-3"
      name="banknote-3"
      src={serialitySrcs.banknote3}
    />,
  ],
  [
    <NamedImage key="plate-1" name="plate-1" src={serialitySrcs.plate} />,
    <NamedImage key="plate-2" name="plate-2" src={serialitySrcs.plate2} />,
    <NamedImage key="plate-3" name="plate-3" src={serialitySrcs.plate3} />,
  ],
  [
    <NamedImage key="paper-1" name="paper-1" src={serialitySrcs.paper} />,
    <NamedImage key="paper-2" name="paper-2" src={serialitySrcs.paper2} />,
    <NamedImage key="paper-3" name="paper-3" src={serialitySrcs.paper3} />,
  ],
]
